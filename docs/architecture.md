...
├── core/         类型、ID 和时间工具
├── playback/     队列、片段和播放状态机
├── storage/      GM 歌单存储、会话存储和数据迁移
...
歌单、歌曲和音量保存在共享 GM 存储中；当前歌单、播放模式、当前歌曲、进度和恢复播放请求则保存在本标签页的 `sessionStorage`。因此刷新或跨 BV 切歌会保留本标签页的播放会话，关闭标签页后会话自然结束；多个标签页可同时播放各自的插件歌单。
...
不同标签页的播放会话彼此独立，不会互相暂停或覆盖进度。共享歌单发生远端变更时，若本标签页正在播放的歌曲或当前歌单被删除，播放器会安全退出歌单播放；其他歌单变更不影响本标签页的播放会话。普通 Bilibili 视频始终使用 `page` 上下文，插件歌单使用 `playlist` 上下文，该上下文只决定本标签页的播放行为和“播放完整视频”操作。
